...
setInterval(() => {
  http.get(`http://miam-games-bot.glitch.me/`);
}, 280000);
...
const devs = process.env.id;
////////////////////|
const ffs = require("fs"); // fs Package //
...
client.on ('message', async (toxicc) => {
  ...
  if (!prefixes[toxicc.guild.id])
    prefixes[toxicc.guild.id] = {
      prefix: "!"
    };
  var prefix = prefixes[toxicc.guild.id].prefix;
